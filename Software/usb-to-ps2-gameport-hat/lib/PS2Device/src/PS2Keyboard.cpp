...
void PS2Keyboard::task() {
  ...
  // typematic handling
  if (last_action.type == KbActionMake && requires_typematic_handling(last_action.code)) {
    // wait for typematic delay
    if (typematic_delay_timer.getElapsedMillis() < typematic_delay_table[typematic_delay]) {
      return;
    }
    // wait for typematic rate
    if (typematic_rate_timer.getElapsedMillis() < typematic_rate_table[typematic_rate]) {
      return;
    }
